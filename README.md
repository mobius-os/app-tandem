...
> **Icon note:** The icon follows the catalog's glossy-3D infinity-motif pipeline (`icon-v6.png`), with the original beige version kept as `icon.png`.
...
4. Tap any word to visibly highlight its sentence in both texts. Exact glossary entries also highlight the true translated word directly in the other text and show a compact pair/note card at the bottom. That card contains only extra glossary information—it never repeats the sentence context—and no positional matches are guessed. Tap the same word (or anywhere else) to clear.
